...
pub(crate) fn publish_immutable(temporary: &Path, destination: &Path) -> io::Result<()> {
...
/// Writes freshly generated bytes through the same temp-plus-atomic-publish path
/// `import_file` uses, so a reader never observes a partial artifact and an
/// already-published version is never replaced.
pub fn write_immutable(destination: &Path, bytes: &[u8]) -> io::Result<()> {
    let parent = destination
        .parent()
        .ok_or_else(|| invalid_path("destination has no parent"))?;
    fs::create_dir_all(parent)?;
...
mod tests {
    ...
    use super::{import_file, publish_immutable, resolve_contained, write_immutable};
    ...
    #[test]
    fn write_immutable_publishes_once_and_leaves_no_temporaries() {
        let root = std::env::temp_dir().join(format!("main-footage-write-{}", uuid::Uuid::new_v4()));
        let destination = root.join("narration/timeline.json");

        write_immutable(&destination, b"{\"schema_version\":1}").unwrap();
        assert_eq!(fs::read(&destination).unwrap(), b"{\"schema_version\":1}");
        assert!(write_immutable(&destination, b"{}").is_err());
        assert_eq!(fs::read(&destination).unwrap(), b"{\"schema_version\":1}");
