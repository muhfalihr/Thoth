...
export function WorkflowMonitor({ workflowId, client }: { workflowId: string | null; client: MonitorClient }) {
  ...
  return <div className="m-3 flex max-w-2xl flex-col gap-3">
    ...
    {approval && <Card><CardHeader><CardTitle>Needs decision</CardTitle></CardHeader><CardContent className="space-y-2"><p>{approval.prompt}</p><div className="flex gap-2">{approval.allowed_decisions.map((decision) => <Button key={decision} size="sm" onClick={() => void applyAction(() => client.approveWorkflow(workflowId, { approval_id: approval.approval_id, decision }))}>{decision}</Button>)}</div></CardContent></Card>}
