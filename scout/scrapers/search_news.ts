...
async function doChart(client, chartQuery) {
  ...
  if (!tagged) { console.log(ui.amber(`  ${ui.WARN} widget kurs tak ketemu (mungkin Google tak menampilkan converter utk query ini).`)); return null; }
  ...
  if (!img) { console.log(ui.amber(`  ${ui.WARN} crop chart gagal/blank.`)); return null; }
...
async function doArticles(client, max) {
  ...
  for (const it of items) {
    ...
    console.log(`  ${it.idx + 1}. ${img ? ui.gold(ui.OK) : ui.amber(`${ui.WARN} (no crop)`)} ${it.title.slice(0, 60)} — ${it.source}`);
  }
  return out;
}
...
run(async () => {
  ...
  if (APPEND) {
    if (!fs.existsSync(APPEND)) { console.log(ui.amber(`${ui.WARN}  --append: ${APPEND} tak ada, lewati.`)); return; }
    let set; try { set = JSON.parse(fs.readFileSync(APPEND, 'utf8')); } catch (e) { console.log(ui.amber(`${ui.WARN}  --append: JSON tak valid.`)); return; }
