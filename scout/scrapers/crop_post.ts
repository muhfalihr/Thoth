...
async function cropPost({ url, out, pad = 8, navWaitMs = 6000, tries = 12, maxSlides = 1, log = (..._a: any[]) => {} }: { url: string; out?: string; pad?: number; navWaitMs?: number; tries?: number; maxSlides?: number; log?: (...a: any[]) => void }) {
  ...
  try {
    ...
    const captureToFile = async (dest) => {
      ...
      if (m.y >= 0 && (m.y + m.h) <= m.ih) {
        ...
        try { execSync(`"${FFMPEG}" -i "${tmp}" -vf "crop=${cw}:${ch}:${cx}:${cy}" -y "${dest}"`, { stdio: 'pipe', timeout: 15000 }); } catch (e) {}
    ...
    const slideKind = async () => client.evaluate(`(() => { const c = document.querySelector('[data-crop-post="1"]'); if (!c) return 'photo'; let big = null, area = 0; c.querySelectorAll('video,img').forEach(m => { const b = m.getBoundingClientRect(); const a = b.width * b.height; if (a > area && b.width > 120) { area = a; big = m; } }); return (big && big.tagName.toLowerCase() === 'video') ? 'video' : 'photo'; })()`).catch(() => 'photo');
    ...
    const slides = [];
    {
      ...
      if (k1 === 'video') { try { fs.unlinkSync(outFile); } catch (e) {} slides.push({ kind: 'video', index: 1 }); }
    ...
    if (maxSlides > 1 && platform === 'instagram') {
      for (let s = 1; s < maxSlides; s++) {
        const clicked = await client.evaluate(`(() => { const b = document.querySelector('button[aria-label="Next"], [aria-label="Next"][role="button"]'); if (b) { b.click(); return 1; } return 0; })()`).catch(() => 0);
        if (!clicked) break;
        await sleep(900);
        // Re-tag if the SPA wiped our marker, then read the rect.
        await client.evaluate(`(() => { if (!document.querySelector('[data-crop-post="1"]')) { const el = ${cfg.find}; if (el) el.setAttribute('data-crop-post','1'); } })()`).catch(() => {});
        if ((await slideKind()) === 'video') { slides.push({ kind: 'video', index: s + 1 }); log(`  slide ${s + 1} (video) ✓`); continue; }
        ...
        if (b2 && okCrop(b2)) { slides.push({ kind: 'photo', index: s + 1, image_path: dest }); log(`  slide ${s + 1} ✓`); } else break;
      }
    }
    const images = slides.filter(s => s.kind === 'photo').map(s => s.image_path);
    return { ok: true, platform, image_path: images[0] || null, images, slides, w: Math.round(m.w), h: Math.round(m.h), bytes: buf.length, text };
  } finally {
    client.close();
  }
}
...
if (import.meta.main) {
  ...
  run(async () => {
    ...
    if (r.ok) { console.log(ui.gold(`${ui.OK} Crop → ${r.image_path} (${r.w}x${r.h}, ${(r.bytes / 1024).toFixed(1)} KB)`)); console.log('image_path:', r.image_path); }
