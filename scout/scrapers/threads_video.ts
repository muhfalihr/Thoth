...
function downloadThreads(vurl, outFile) {
  ...
  try {
    execSync(`"${YTDLP}" --no-warnings -o "${outFile}" "${vurl.replace(/"/g, '%22')}"`, {
      stdio: 'pipe',
      timeout: 120000,
    });
  } catch (e) {
...
if (import.meta.main) {
  ...
  if (!url) {
    console.log('Usage: bun threads_video.ts <threads_post_url> [--download]');
    process.exit(1);
  }
  run(async () => {
    const src = await threadsVideoSrc(url);
    if (!src) {
      console.log(ui.amber(`${ui.WARN}  tak ada <video> di post ini (mungkin post teks/foto).`));
      return;
    }
