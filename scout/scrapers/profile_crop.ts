...
async function captureRect(c, rect, outPng, { pad = 12, beyondViewport = false } = {}) {
  ...
  if (beyondViewport) {
    try {
      ox = (await c.evaluate('window.scrollX')) || 0;
      oy = (await c.evaluate('window.scrollY')) || 0;
    } catch (e) {}
  }
  // page-coords rect (viewport + scroll) → device-pixel clip handled by captureClip (dpr-correct).
  const data = await c.captureClip({ x: rect.x + ox, y: rect.y + oy, w: rect.w, h: rect.h }, pad, {
    beyondViewport,
  });
  if (!data) return '';
...
function igResolveHandle(url) {
  ...
  try {
    const out = execFileSync(
      YTDLP,
      ['--skip-download', '--no-warnings', '--print', '%(channel)s', '--', url],
...
async function cropInstagram(c, user, outPng) {
  ...
  await sleep(2500);
  for (let i = 0; i < 8; i++) {
    if (await c.evaluate(`!!document.querySelector('main header, header')`)) break;
    await sleep(800);
  }
  try {
  ...
  if (!rect) {
    console.log('    [instagram] crop profil: header tak terbaca.');
    return '';
  }
...
function overlayCircleAvatar(base, avatar, ax, ay, d, outPng) {
  ...
  try {
    execFileSync(
      FFMPEG,
      ['-y', '-i', base, '-i', avatar, '-filter_complex', fc, '-frames:v', '1', outPng],
...
async function cropTwitter(c, user, outPng) {
  ...
  for (let i = 0; i < 10; i++) {
    if (
      await c.evaluate(`!!document.querySelector('[data-testid="primaryColumn"] [role="tablist"]')`)
    )
      break;
    await sleep(700);
  }
  ...
  for (let i = 0; i < 12; i++) {
    const mw = await c.evaluate(
      `(() => { const col = document.querySelector('[data-testid="primaryColumn"]'); if (!col) return 0; let mw = 0; col.querySelectorAll('[data-testid^="UserAvatar-Container"]').forEach(a => { const w = a.getBoundingClientRect().width; if (w > mw) mw = w; }); return mw; })()`,
    );
    if (mw > 100) break;
    await sleep(600);
  }
  ...
  let m;
  try {
    m = JSON.parse(data || 'null');
  } catch (e) {}
  if (!m) {
    console.log('    [twitter] crop profil: header tak terbaca.');
    return '';
  }

  const pad = 12;
  const base = await captureRect(c, { x: m.cropX, y: m.cropY, w: m.cropW, h: m.cropH }, outPng, {
    pad,
  });
  if (!base) return '';
  ...
  if (avFile) {
    const ox = Math.max(0, m.cropX - pad),
      oy = Math.max(0, m.cropY - pad);
    const ax = (m.avX - ox) * dpr,
      ay = (m.avY - oy) * dpr,
      d = Math.round(Math.min(m.avW, m.avH) * dpr);
    console.log(
      `    [twitter] dbg dpr=${dpr} col.left=${Math.round(m.cropX)} av=(${Math.round(m.avX)},${Math.round(m.avY)},${Math.round(m.avW)}) → ax=${Math.round(ax)} ay=${Math.round(ay)} d=${d}`,
    );
    const composed = overlayCircleAvatar(base, avFile, ax, ay, d, outPng + '.tmp.png');
    try {
      fs.unlinkSync(avFile);
    } catch (e) {}
    if (composed) {
      try {
        fs.renameSync(composed, outPng);
      } catch (e) {}
...
async function cropProfile(
  platform: string,
  username: string,
  outPng: string,
  { client, url }: { client?: any; url?: string } = {},
) {
  platform = (platform || '').toLowerCase();
  if (!MATCH[platform] || !outPng) return '';
  if (platform === 'tiktok') {
    const { cropTiktokProfile } = await import('./tiktok_profile.ts');
    return cropTiktokProfile(username, outPng, { client });
  }
  ...
  if (!c) {
    try {
      c = await connect({ match: MATCH[platform], requireMatch: true });
      own = true;
    } catch (e) {
      return '';
    }
  ...
  try {
    ...
    if (platform === 'instagram') {
      ...
      if (!user) {
        console.log('    [instagram] crop profil: handle tak diketahui.');
        return '';
      }
    ...
    if (platform === 'twitter') {
      ...
      if (process.env.THOTH_PROFILE_X !== '1') {
        console.log(
          '    [twitter] crop profil WIP (avatar occluded-tab) → kartu sintetis. Set THOTH_PROFILE_X=1 utk paksa.',
        );
        return '';
      }
      if (!user) {
        console.log('    [twitter] crop profil: handle tak diketahui.');
        return '';
      }
...
if (import.meta.main) {
  ...
  if (!platform || !out) {
    console.log('Usage: bun profile_crop.ts <platform> <username|""> <out.png> [reelUrl]');
    process.exit(1);
  }
  ...
  cropProfile(platform, username, out, { url }).then((r) => {
    if (r) console.log(ui.gold(`${ui.OK} crop → ${r}`));
      ...
      console.log(ui.amber(`${ui.WARN} gagal`));
      process.exit(2);
    }
  });
}
