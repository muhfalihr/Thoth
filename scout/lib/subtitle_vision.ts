...
export function mainDirectiveFields(
  verdict: ClipVerdict,
): Pick<ClipVerdict, 'trim_start' | 'mute_audio' | 'subtitle_blur'> {
  return {
...
export function parseDeepSeekOcr(content: string): OcrBox[] {
  ...
  const append = (rawText: string, encodedBoxes: string) => {
    ...
    for (const raw of boxes) {
      ...
      if (x1 - x0 >= 0.01 && y1 - y0 >= 0.01) out.push({ text, x0, y0, x1, y1 });
    }
  };
  ...
  if (out.length === 0) {
    const plainPair =
      /^\s*(.*?)\s*(\[\[\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*,\s*-?\d+(?:\.\d+)?\s*\]\])\s*$/gm;
...
export function buildSampleTimes(duration: number, maxFrames = 12): number[] {
  ...
  const intro = [0.5, 1, 2, 3, 4, 5].filter((t) => t < duration).slice(0, limit);
  if (intro.length >= limit || duration <= 5) return intro;

  const remaining = limit - intro.length;
  const safeEnd = Math.max(5, duration - Math.min(0.1, duration / 100));
...
function verticalOverlap(a: OcrBox, b: OcrBox): number {
  ...
  return overlap / Math.max(0.000001, Math.min(a.y1 - a.y0, b.y1 - b.y0));
}

function horizontalOverlap(a: OcrBox, b: OcrBox): number {
  const overlap = Math.max(0, Math.min(a.x1, b.x1) - Math.max(a.x0, b.x0));
  return overlap / Math.max(0.000001, Math.min(a.x1 - a.x0, b.x1 - b.x0));
}
...
function groupSpatialBoxes(boxes: OcrBox[]): OcrBox[][] {
  ...
  for (const box of boxes) {
    const connected = groups.flatMap((group, index) =>
      group.some((other) => {
        const horizontalGap = axisGap(box.x0, box.x1, other.x0, other.x1);
        const verticalGap = axisGap(box.y0, box.y1, other.y0, other.y1);
        return (
          (verticalOverlap(box, other) >= 0.5 && horizontalGap <= 0.08) ||
          (horizontalOverlap(box, other) >= 0.3 && verticalGap <= 0.07)
        );
      })
        ? [index]
        : [],
...
function hasMultilineLayout(boxes: OcrBox[]): boolean {
  return boxes.some((box, index) =>
    boxes.some((other, otherIndex) => {
      if (index === otherIndex) return false;
      const gap = Math.max(0, Math.max(box.y0, other.y0) - Math.min(box.y1, other.y1));
      return gap <= 0.07 && horizontalOverlap(box, other) >= 0.3;
    }),
  );
}

function isEditorialLabel(text: string): boolean {
  return /^(breaking news|live(?: news)?|exclusive|news(?: update)?|sports|weather|headlines?)$/.test(
    normText(text),
  );
}
...
function isLikelyLowerThird(
  ...
  if (box.y0 < 0.65 || box.x0 > 0.3) return false;
  ...
  const hasCompanionLabel = editorialLabels.some((label) => {
    ...
    return (
      normText(label.text) !== normText(box.text) &&
      Math.abs(labelCenterY - boxCenterY) <= 0.12 &&
      label.x0 < box.x0 - 0.03 &&
      boxIou(label, box) < 0.2
    );
  });
  if (!hasCompanionLabel) return false;

  const track = frames.flatMap((frame, index) =>
    frame.boxes
      .filter((other) => verticalOverlap(box, other) >= 0.5 && Math.abs(other.x0 - box.x0) <= 0.04)
  ...
  return (
    Math.max(...lefts) - Math.min(...lefts) <= 0.035 &&
    track.some(
      (match) => match.index !== frameIndex && normText(match.box.text) !== normText(box.text),
    )
  );
}
...
function envelope(boxes: OcrBox[]): OcrBox {
  const x0 = Math.max(0, Math.min(...boxes.map((box) => box.x0)) - 0.02);
  const y0 = Math.max(0, Math.min(...boxes.map((box) => box.y0)) - 0.015);
  const x1 = Math.min(1, Math.max(...boxes.map((box) => box.x1)) + 0.02);
  const y1 = Math.min(1, Math.max(...boxes.map((box) => box.y1)) + 0.015);
...
export function classifyOcrFrames(frames: OcrFrame[], duration: number): ClipVerdict {
  ...
  const stableThreshold = Math.max(2, Math.ceil(usableCount * 0.6));
  ...
  for (let frameIndex = 0; frameIndex < filtered.length; frameIndex++) {
    ...
    for (const anchor of frame.boxes.filter(
      (box) => boxArea(box) >= 0.015 && box.x1 - box.x0 >= 0.45,
    )) {
      const matchingIndexes: number[] = [];
      for (let i = frameIndex; i < filtered.length && filtered[i].t <= COVER_MAX; i++) {
        if (
          filtered[i].boxes.some(
            (box) => textSimilarity(anchor.text, box.text) >= 0.5 && boxIou(anchor, box) >= 0.45,
          )
        ) {
      ...
      const absentIndex = filtered.findIndex(
        (later, i) =>
          i > lastIndex &&
          !later.boxes.some(
            (box) => textSimilarity(anchor.text, box.text) >= 0.5 && boxIou(anchor, box) >= 0.45,
  ...
  const candidateFrames = rawCandidateFrames.map((frame, frameIndex) => ({
    ...frame,
    boxes: frame.boxes.filter(
      (box) => !isLikelyLowerThird(box, frameIndex, rawCandidateFrames, editorialLabels),
    ),
  }));
  const multilineFrames = candidateFrames.map((frame) => hasMultilineLayout(frame.boxes));
  const selectedByFrame = candidateFrames.map((frame, frameIndex) =>
    frame.boxes.filter(
      (box) =>
        candidateFrames.some(
          (other, otherIndex) =>
            ...
            other.boxes.some(
              (otherBox) =>
                verticalOverlap(box, otherBox) >= 0.5 &&
                normText(box.text) !== normText(otherBox.text),
            ),
        ) ||
        (multilineFrames[frameIndex] &&
          candidateFrames.some(
            (other, otherIndex) =>
              otherIndex !== frameIndex &&
              multilineFrames[otherIndex] &&
              other.boxes.some((otherBox) => verticalOverlap(box, otherBox) >= 0.5),
          )),
    ),
  );
  ...
  for (const window of windows) {
    ...
    for (const candidate of merged) {
      ...
      if (iou < 0.6 && (vertical < 0.5 || horizontal < 0.5)) continue;
...
export function buildClassifiedDiagnostics(
  ...
  return frames.map((frame) => {
    ...
    const headline_boxes =
      verdict.trim_start > 0 && frame.t < verdict.trim_start
        ? frame.boxes.filter((box) => boxArea(box) >= 0.015 && box.x1 - box.x0 >= 0.45)
        : [];
...
export function classifyClip(frames: FrameDet[], duration: number): ClipVerdict {
  ...
  if (lastText <= COVER_MAX && cleanAfter) {
    return {
      outcome: 'cover',
      trim_start: (lastText + cleanAfter.t) / 2,
      mute_audio: false,
      subtitle_blur: [],
    };
  }
  ...
  if (allText) {
    ...
    for (const w of wins) {
      ...
      if (last && w.s <= last.e) {
        ...
        last.r = {
          x0: Math.min(last.r.x0, w.r.x0),
          y0: Math.min(last.r.y0, w.r.y0),
          x1: Math.max(last.r.x1, w.r.x1),
          y1: Math.max(last.r.y1, w.r.y1),
        };
      } else merged.push({ ...w });
    }
    wins = merged;
...
function frameDataUrl(
  ...
  const tmp = path.join(
    os.tmpdir(),
    `subv_${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`,
  );
  try {
    execFileSync(
      ffmpeg,
...
export function parseVisionFrame(resp: string): { present: boolean; region: FrameDet['region'] } {
  ...
  try {
    ...
    const a = o.region;
    const region =
      Array.isArray(a) && a.length === 4 && a.every((n: any) => typeof n === 'number')
        ? normalizeRegion(a[0], a[1], a[2], a[3])
        : null;
    return { present: true, region };
  } catch {
    return { present: false, region: null };
  }
}
...
export function classifyVisionText(resp: string): boolean {
  ...
  const m = (resp || '').match(/\{[\s\S]*?\}/);
  try {
    return !!m && JSON.parse(m[0]).reject === true;
  } catch {
    return false;
  }
}
...
function safeErrorMessage(errorCode: string): string {
  switch (errorCode) {
    ...
    case 'missing_api_key':
      return 'OCR API key is not configured';
    case 'duration_probe_failed':
      return 'Video duration could not be determined';
    case 'ffprobe_missing':
      return 'ffprobe executable was not found';
    case 'incomplete_frame_coverage':
      return 'OCR did not analyze every scheduled frame';
    default:
...
const FRAME_RECOVERY_OFFSETS = [0, 0.25, 0.5, 1] as const;
...
function frameRecoveryTimes(requested: number, previous: number | undefined): number[] {
  ...
  return [...new Set(candidates)].filter(
    (candidate) => previous === undefined || candidate > previous,
  );
}
...
export async function analyzeSubtitlesDetailed(
  ...
  if (probe.status === 'failed') {
    return failedAnalysis(
      ...
      { configured: retryCount, actual: 0 },
      {
        ...
        ...(probe.safe_exit_code !== undefined ? { probe_exit_code: probe.safe_exit_code } : {}),
      },
    );
  }
  ...
  appendAnalysisDiagnostics(videoUrl, resolvedDuration, result, frames, writeDiagnostics, {
    configured: retryCount,
    actual: actualRetryCount,
  });
  return result;
}
...
export async function analyzeSubtitles(
  ...
  const analysis = await runRequiredOcr(() => analyzeSubtitlesDetailed(videoUrl, duration, deps));
