...
const CONFIG = {
  ...
  outputDir: CROPS_DIR,
  novitaKey: fs.existsSync(path.join(__dirname, '..', '.novita_key'))
    ? fs.readFileSync(path.join(__dirname, '..', '.novita_key'), 'utf8').trim()
    : 'sk-placeholder',
...
if (args.length < 1) {
  ...
  console.error(
    '  node crop_comment_pipeline.js https://www.tiktok.com/@kompascom/video/7647116110399048980',
  );
  console.error(
    '  node crop_comment_pipeline.js https://www.tiktok.com/@user/video/123 content_set.json\n',
  );
  process.exit(1);
}
...
async function detect(ssPath) {
  ...
  execSync(
    `"${CONFIG.ffmpeg}" -i "${ssPath}" -vf "scale=${CONFIG.visionWidth}:-1" -update 1 -y "${resizedPath}"`,
    { stdio: 'pipe', timeout: 10000 },
  );
  const vis = pngSize(resizedPath); // actual resized dims (width x height)
  console.log(`  Image dikirim: ${vis.w}x${vis.h}`);

  const { comments, raw } = await detectComments({
    imagePath: resizedPath,
    key: CONFIG.novitaKey,
...
function cropComments(ssPath, comments, vis) {
  ...
  comments.forEach((c, i) => {
    ...
    let w = x2 - x1,
      h = y2 - y1;
    if (w < 30 || h < 20) {
      console.log(`  ⏭️  #${i + 1} box terlalu kecil (${w}x${h}), skip`);
      return;
    }
    ...
    const finalPath = path.join(
      CONFIG.outputDir,
      `comment_${String(i + 1).padStart(2, '0')}_${safeName}_${c.likes}like.png`,
    );

    try {
      // 1) crop the natural comment block, 2) scale to display width keeping aspect.
      execSync(
        `"${CONFIG.ffmpeg}" -i "${ssPath}" -vf "crop=${w}:${h}:${x1}:${y1}" -update 1 -y "${tmp}"`,
        { stdio: 'pipe', timeout: 10000 },
      );
      execSync(
        `"${CONFIG.ffmpeg}" -i "${tmp}" -vf "scale=${CONFIG.displayWidth}:-2" -update 1 -y "${finalPath}"`,
        { stdio: 'pipe', timeout: 10000 },
      );
...
function saveThothFormat(url, results) {
  ...
  if (fs.existsSync(THOTH_OUTPUT)) {
    try {
      existing = JSON.parse(fs.readFileSync(THOTH_OUTPUT, 'utf8'));
...
async function main() {
  ...
  if (!comments.length) {
    console.log('\n⚠️ Tidak ada komentar terdeteksi.');
    return;
  }

  const results = cropComments(ssPath, comments, vis);
  if (results.length === 0) {
    console.log('\n⚠️ Tidak ada komentar yang berhasil di-crop.');
    return;
  }
  ...
  if (viral.length > 0) {
    ...
    viral.forEach((r) =>
      console.log(`  → @${r.user} (${r.likes} likes) — "${r.text.slice(0, 50)}"`),
    );
  }
