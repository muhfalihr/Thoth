    ...
    b.push(
      '[Tokoh]\n' +
        s.figures
          .map(
            (f) =>
              `- ${f.name}${f.role ? ' (' + f.role + ')' : ''}${f.description ? ': ' + f.description : ''}`,
          )
          .join('\n'),
    );
  if (s.comments && s.comments.length) {
    const top = s.comments
      .slice()
      .sort((a, c) => (c.likes || 0) - (a.likes || 0))
      .slice(0, 12)
      .map((c) =>
        c.likes ? `- ${c.author} (${c.likes} like): ${c.text}` : `- ${c.author}: ${c.text}`,
      );
...
const userPrompt = (
  ...
  words,
  secs,
) => `Ini SUMBER KONTEKS sebuah kejadian (blok [Judul]/[Deskripsi]/[Tokoh]/[Komentar Netizen Teratas]). Pahami TOPIK & FAKTA nyatanya dari SEMUA blok:
====================
${src}
====================
