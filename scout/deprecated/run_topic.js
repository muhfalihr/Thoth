...
//
// ONE command for the whole scout upstream: a topic → a validated Thoth
...
const { outPath } = require('../lib/paths');
...
const QUERY = args.find(
  (a) =>
    !a.startsWith('--') &&
    !['--platforms', '--max', '--main', '--keywords', '--mode'].includes(args[args.indexOf(a) - 1]),
);
...
if (!QUERY) {
  console.log(
    'Usage: node deprecated/run_topic.js "<topik>" [--platforms tiktok,tw,ig,fb] [--max N] [--main <url>] [--no-enrich]',
  );
  process.exit(1);
}
...
const slug =
  QUERY.toLowerCase()
    ...
    .replace(/^_|_$/g, '')
    .slice(0, 40) || 'topic';
const here = (f) => path.join(__dirname, '..', 'pipeline', f); // pipeline scripts (post-2026-07 restructure)
...
try {
  step(1, 'Topik → URL lintas platform (+gate TikTok di sumber)', 'topic_to_urls.js', [
    QUERY,
    '--platforms',
    PLATFORMS,
    '--max',
  ...
  const urlArgs = [topicUrls, '--keywords', KEYWORDS, '--mode', MODE];
  if (MAIN) urlArgs.push('--main', MAIN);
  ...
  if (NO_TRACE) console.log('\n[2a] trace source di-SKIP (--no-trace).');
    ...
    try {
      step('2a', 'Trace source (anti re-wrap, MAIN)', 'trace_source.js', [
        contentSet,
        '--keywords',
        KEYWORDS,
      ]);
    } catch (e) {
      console.log('  ⚠️ trace dilewati: ' + String((e && e.message) || e).slice(0, 60));
    }
  }
  ...
  if (NO_NEWS) console.log('\n[2b] news di-SKIP (--no-news).');
    ...
    try {
      step('2b', 'News (chart/artikel) → footage', 'search_news.js', [
        QUERY,
        '--append',
        contentSet,
        '--max',
      ...
      console.log('  ⚠️ news dilewati: ' + String((e && e.message) || e).slice(0, 60));
    }
  }
  ...
  if (NO_ENRICH)
    ...
    step(3, 'Enrich image_path + gate relevansi non-video', 'enrich_image_paths.js', [
      contentSet,
      '--keywords',
      KEYWORDS,
      '--mode',
