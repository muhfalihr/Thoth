...
const technical = {
  container: 'mp4', video_codec: 'h264', duration_sec: 12.5, width: 1080, height: 1920, has_audio: true,
};
...
try {
  ...
  const result = await buildSourcePackage(
    { post: post([photo(0), video(1), photo(2), video(3)]), contentSetPath, coverageTarget: 0.75 },
    {
      ...
      materialize: async (asset) => {
        ...
        return { path: writeDownload('video-1.mp4'), kind: 'video', source: 'direct-http', bytes: acceptedBytes.length };
      },
  ...
  assert.deepEqual(result.package.sources.map((source) => source.media_index), [1]);
  ...
  assert.equal(result.package.sources[0]?.checksum, `sha256:${createHash('sha256').update(acceptedBytes).digest('hex')}`);
  ...
  assert.deepEqual(result.package.sources[0]?.acquisition, { source: 'direct-http', attempts: 1, elapsed_ms: 0 });
  ...
  const second = await buildSourcePackage(
    ...
    {
      scoutOutputRoot,
      materialize: async () => ({ path: writeDownload('video-1-repeat.mp4'), kind: 'video', source: 'direct-http', bytes: acceptedBytes.length }),
      probe: async () => technical,
      now: () => 100,
    },
  );
  assert.notEqual(second.packagePath, result.packagePath, 'a published package is never overwritten');
  assert.deepEqual(fs.readFileSync(path.join(path.dirname(second.packagePath), second.package.sources[0]!.path)), acceptedBytes);

  const everyAttempt: number[] = [];
  await assert.rejects(
    () => buildSourcePackage(
      { post: post([video(1), video(3)]), contentSetPath, coverageTarget: 0.60 },
      {
        scoutOutputRoot,
        materialize: async (asset) => {
  ...
  const interrupted = await assert.rejects(
    () => buildSourcePackage(
      { post: post([video(5)]), contentSetPath, coverageTarget: 0.60 },
      {
        scoutOutputRoot,
        materialize: async () => ({ path: writeDownload('interrupted.mp4'), kind: 'video', source: 'direct-http', bytes: acceptedBytes.length }),
        probe: async () => { throw new Error('ffprobe interrupted'); },
      },
    ),
    { message: 'forced_main_no_usable_video' },
  );
