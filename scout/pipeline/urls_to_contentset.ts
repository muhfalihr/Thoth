...
const IN = args.find(
  (a) =>
    !a.startsWith('--') &&
    args[args.indexOf(a) - 1] !== '--main' &&
    args[args.indexOf(a) - 1] !== '--out',
);
const MAIN_OVERRIDE = getFlag('--main');
if (!IN) {
  console.log('Usage: bun urls_to_contentset.ts <topic_urls_*.json> [--main <url>] [--out <file>]');
  process.exit(1);
}
if (!fs.existsSync(IN)) {
  console.log(ui.red(`${ui.ERR} File tak ada: ${IN}`));
  process.exit(1);
}

let src;
try {
  src = JSON.parse(fs.readFileSync(IN, 'utf8'));
} catch (e) {
...
if (!all.length) {
  console.log(ui.red(`${ui.ERR} Tak ada URL di field \`all\`.`));
  process.exit(1);
}
...
(async () => {
  ...
  if (MAIN_OVERRIDE) {
    ...
    for (const v of vids) {
      const cap = await videoCaption(v);
      if (cap && matchesTopic(cap, KEYWORDS, MODE)) {
        main = v;
        mainCaption = cap;
        break;
      }
    ...
    if (!main) {
      main = vids[0] || all[0];
      if (main && VIDEO.has(main.platform)) {
        mainCaption = await videoCaption(main);
        console.log(
          ui.amber(
            `${ui.WARN}  Tak ada video yg caption-nya match [${KEYWORDS.join(', ')}] — pakai video pertama (mungkin off-topic): ${main.url}`,
          ),
        );
      }
    }
  ...
  if (!main) {
    console.log(ui.red(`${ui.ERR} Tak ada kandidat main.`));
    process.exit(1);
  }
  ...
  if (!mainIsVideo)
    console.log(
      ui.amber(
        `${ui.WARN}  main bukan video (${main.platform}) — Thoth butuh main yg bisa di-ingest yt-dlp. Pertimbangkan --main <url tiktok/youtube>.`,
      ),
    );
  ...
  console.log(
    `Main    : [${mainObj.platform}${mainObj.is_video ? '' : ui.amber(` ${ui.WARN}non-video`)}] ${mainObj.url}`,
  );
  console.log(`          "${title.slice(0, 70)}"`);
  console.log(
    `Footage : ${footage.length} (${vid} video cutaway [${matched} on-topic/"match"] + ${img} non-video → image card; relevance non-video di-gate saat enrich)`,
  );
