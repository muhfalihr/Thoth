...
const FETCHERS = {
  tiktok: () => runFetcher(['search_tiktok_v2.ts', QUERY, ...(TT_KW ? [TT_KW] : [])], outPath('tiktok_urls.json')),
...
(async () => {
  ...
  for (const p of PLATFORMS) {
    ...
    try {
      ...
      platforms[p] = [];
      const relay = (err && (err.relay || /1880\d|18792|ECONNREFUSED|terbuka|CDP/.test(String(err.message || err))));
      console.log(ui.amber(relay
  ...
  const all = [];
  for (const [p, urls] of Object.entries<any>(platforms)) urls.forEach(url => all.push({ platform: canon[p] || p, url }));
