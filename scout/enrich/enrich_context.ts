...
async function enrich(set) {
  if (!KEY) { console.log('  (no THOTH_NOVITA_API_KEY → skip enrich, set unchanged)'); return false; }
  const main = set.main || {};
  ...
  try {
    ...
    if (!resp.ok) { console.log(ui.amber(`  ${ui.WARN} LLM ${resp.status} → skip`)); return false; }
    const d = await resp.json();
    txt = (d.choices && d.choices[0] && d.choices[0].message && d.choices[0].message.content) || '';
  } catch (e) { console.log(ui.amber(`  ${ui.WARN} LLM error → skip: ${String(e.message || e).slice(0, 60)}`)); return false; }

  const m = txt.match(/\{[\s\S]*\}/); if (!m) { console.log(ui.amber(`  ${ui.WARN} no JSON → skip`)); return false; }
  let o; try { o = JSON.parse(m[0]); } catch (e) { console.log(ui.amber(`  ${ui.WARN} JSON parse fail → skip`)); return false; }
  ...
  console.log(ui.gold(`  ${ui.OK} ${refs.length} ref, ${tagged} komentar di-decode, stance="${(set.discourse.audience_stance || '').slice(0, 60)}"`));
  return true;
}
...
(async () => {
  ...
  console.log(ui.rule());
  let set; try { set = JSON.parse(fs.readFileSync(FILE, 'utf8')); } catch (e) { console.log(ui.red(`${ui.ERR} JSON tak valid: ${FILE}`)); process.exit(0); }
