...
function help(bad?: string) {
  ...
  for (const [k, [, desc]] of Object.entries(CMDS))
    console.log(`${spine}${ui.gold(k.padEnd(w))}  ${ui.dim(desc)}`);
  console.log(spine);
  console.log(
    `${spine}${ui.violet('Flow harian:')} ${ui.dim('browser status → discover → run <url> → validate → thoth run --content')}`,
  );
...
const r = spawnSync(process.execPath, [path.join(import.meta.dirname, CMDS[cmd][0]), ...rest], {
  stdio: 'inherit',
});
