...
if (!IN) { console.log('Usage: node urls_to_contentset.js <topic_urls_*.json> [--main <url>] [--out <file>]'); process.exit(1); }
if (!fs.existsSync(IN)) { console.log('❌ File tak ada:', IN); process.exit(1); }

let src; try { src = JSON.parse(fs.readFileSync(IN, 'utf8')); } catch (e) { console.log('❌ JSON tak valid:', e.message); process.exit(1); }
const query = src.query || 'topik';
const all = Array.isArray(src.all) ? src.all : [];
if (!all.length) { console.log('❌ Tak ada URL di field `all`.'); process.exit(1); }
...
(async () => {
  ...
  if (MAIN_OVERRIDE) {
    ...
    if (!main) {
      main = vids[0] || all[0];
      if (main && VIDEO.has(main.platform)) { mainCaption = await videoCaption(main); console.log(`⚠️  Tak ada video yg caption-nya match [${KEYWORDS.join(', ')}] — pakai video pertama (mungkin off-topic): ${main.url}`); }
    }
  }
  ...
  console.log(`Main    : [${mainObj.platform}${mainObj.is_video ? '' : ' ⚠️non-video'}] ${mainObj.url}`);
