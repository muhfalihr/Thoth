...
//
//   const { cropProfile } = require('./profile_crop');
...
async function cropProfile(platform, username, outPng, { client, url } = {}) {
  platform = (platform || '').toLowerCase();
  if (!MATCH[platform] || !outPng) return '';
  if (platform === 'tiktok') { const { cropTiktokProfile } = require('./tiktok_profile'); return cropTiktokProfile(username, outPng, { client }); }
...
module.exports = { cropProfile, igResolveHandle };

// ---- CLI (testing) ----
if (require.main === module) {
  const [platform, username, out] = process.argv.slice(2);
  if (!platform || !out) { console.log('Usage: node profile_crop.js <platform> <username|""> <out.png> [reelUrl]'); process.exit(1); }
  const url = process.argv[5];
  cropProfile(platform, username, out, { url }).then(r => {
    if (r) console.log('✅ crop →', r);
    else { console.log('⚠️ gagal'); process.exit(2); }
  });
}
