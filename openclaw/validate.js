  ...
  sets.forEach((set, si) => {
    ...
    if (!main || !main.url) {
      ...
      if (main.is_video === false) {
        ...
        else if (!fs.existsSync(main.image_path)) errors.push(`${tag}.main.image_path tidak ada di disk: ${main.image_path}`);
      }
    ...
    footage.forEach((f, fi) => {
      ...
      if (f.is_video === false) {
        ...
        else if (!fs.existsSync(f.image_path)) errors.push(`${ft}.image_path tidak ada di disk: ${f.image_path}`);
      }
    });
...
module.exports = {
  IG_RE, TW_RE, TK_RE, YT_RE, FB_RE, PLATFORM_RE,
  validIG, validTW, validTK, inferPlatform, validUrlFor, lintContentSet,
};
