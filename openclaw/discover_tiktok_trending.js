...
async function scrapeTopics(client, max, region) {
  ...
  if (region) {
    ...
    else if (reg.changed) console.log(`  ⚠️  region: gagal switch ke ${region} (sekarang ${reg.region || '?'})`);
    else if (reg.ok) console.log(`  region: ${reg.region || '?'} (sudah sesuai)`);
    else console.log(`  ⚠️  region: gagal buka filter — pakai ${reg.region || 'tampilan saat ini'}`);
    await sleep(1000);
  }
...
async function fetchTrending({ max = 25, region = 'Indonesia' } = {}) {
  ...
  try {
    ...
    console.log(`⚠️  TikTok trending gagal: ${String(e.message || e).slice(0, 80)} (butuh tab tiktok.com login)`);
    return [];
  } finally {
...
module.exports = { fetchTrending, scrapeTopics, selectRegion, readRegion, parseViews, TRENDING_URL };
