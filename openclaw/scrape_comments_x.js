...
const { run } = require('./cdp');
const { scrapeComments, parseArgs, pollCount } = require('./comment_engine');
...
if (!url) { console.log('Usage: node scrape_comments_x.js <tweet_url> [out.json] [--max N]'); process.exit(1); }

const id = (url.match(/status\/(\d+)/) || [, ''])[1];
const user = (url.match(/(?:x|twitter)\.com\/([^/?#]+)/) || [, 'user'])[1];

run(() => scrapeComments({
  url, platform: 'twitter', label: 'X/Twitter',
  match: ['x.com', 'twitter.com'], idToken: id,
