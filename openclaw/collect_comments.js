// collect_comments.js — fill content-set comments[] from MULTIPLE sources, not just one post.
//
...
//
//   node collect_comments.js <content_set.json> [--per-source 6] [--cap 12] [--max-sources 4] [--extra url1,url2]
//
...
if (!FILE) { console.log('Usage: node collect_comments.js <content_set.json> [--per-source 6] [--cap 12] [--max-sources 4] [--extra url1,url2]'); process.exit(1); }
...
const SCRIPT = {
  ...
  youtube: 'scrape_comments_yt.js', facebook: 'scrape_comments_fb.js', reddit: 'scrape_comments_reddit.js',
};
...
(async () => {
  ...
  console.log('='.repeat(60));
  if (!sources.length) { console.log('⚠️  Tak ada sumber komentar yang punya scraper (threads/CDN di-skip). Pakai --extra <url>.'); process.exit(0); }
  ...
  for (const s of sources) {
    ...
    try {
      execFileSync('node', [path.join(__dirname, SCRIPT[s.platform]), s.url, tmp, '--max', String(PER_SOURCE)], { stdio: 'pipe', timeout: 180000 });
