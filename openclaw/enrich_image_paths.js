...
const { matchesTopic } = require('./verify');
...
if (!FILE) { console.log('Usage: node enrich_image_paths.js <content_set.json> [--force] [--keywords k1,k2] [--mode any|all]'); process.exit(1); }
if (!fs.existsSync(FILE)) { console.log('❌ File tak ada:', FILE); process.exit(1); }

let set;
try { set = JSON.parse(fs.readFileSync(FILE, 'utf8')); }
catch (e) { console.log('❌ JSON tak valid:', e.message); process.exit(1); }
if (Array.isArray(set) || !set || typeof set !== 'object') { console.log('❌ content-set harus objek tunggal {main,footage,comments}.'); process.exit(1); }
...
(async () => {
  ...
  for (const { label, e } of todo) {
    ...
    try {
      ...
      else { fail++; console.log(`⚠️  ${r.reason}`); }
    } catch (err) {
      fail++;
      if (err && err.relay) console.log(`⚠️  tab ${plat} belum ter-attach relay (skip; attach lalu --force)`);
  ...
  console.log('Validasi: node validate_content_set.js "' + FILE + '"');
})();
