...
const IG_ACCOUNTS_FILE = path.join(__dirname, 'ig_accounts.json');
...
(async () => {
  ...
  for (const p of PLATFORMS) {
    ...
    try {
      ...
      sources[p] = items;
      console.log(`✅ ${items.length}${dropped ? ` (${dropped} sepak bola di-skip)` : ''}`);
      items.slice(0, 8).forEach((it, i) => console.log(
        `    ${i + 1}. ${it.account ? `[@${it.account}·${(it.views || 0).toLocaleString()}] ` : ''}${it.topic}`));
    } catch (err) {
      sources[p] = [];
      if (err && err.relay) console.log(`⚠️  tab ${p} belum ter-attach relay (skip)`);
  ...
  console.log('-'.repeat(60));
  console.log(`📄 ${out}`);
  console.log('Lanjut: node topic_to_urls.js "<topik pilihan>"');
})();
