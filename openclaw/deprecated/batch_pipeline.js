...
if (args.length < 3) {
  console.error('\n❌ Usage:');
  console.error('  node batch_pipeline.js <topic_slug> <topic_name> <main_url> [react_url1 react_url2 ...]');
  console.error('\nExamples:');
...
async function scrapeComments(videoUrl) {
  ...
  try {
    ...
    execSync(`"${FFMPEG}" -i "${ssPath}" -vf "scale=1200:-1" -update 1 -y "${visionPath}"`, { stdio: 'pipe', timeout: 10000 });
    const vis = pngSize(visionPath);

    const { comments } = await detectComments({ imagePath: visionPath, key: NOVITA_KEY, W: vis.w, H: vis.h });
    console.log(`  ${comments.length} comments detected`);
    return comments.map(c => ({ author: '@' + (c.user || 'anon'), text: c.text, likes: c.likes, avatar_url: '' }));
  } catch (e) {
...
async function main() {
  ...
  const contentSet = {
    main: {
      url: MAIN_URL, platform: 'tiktok', title: `[${TOPIC_NAME}] Raw — @${username}`,
      is_video: true, duration_sec: 60,
      profile: { name: username, handle: username, followers: '', avatar_url: '' },
    },
    footage: REACT_URLS.map(url => ({
      platform: 'tiktok', url, title: `Reaction @${extractUsername(url)}`,
      is_video: true, duration_sec: 60, relevance: 'match',
    })),
    comments,
  };
  ...
  if (existing && existing.main?.url === MAIN_URL) {
    ...
    for (const r of REACT_URLS) {
      if (!existingUrls.has(r)) {
        existing.footage.push({ platform: 'tiktok', url: r, title: `Reaction @${extractUsername(r)}`, is_video: true, duration_sec: 60, relevance: 'match' });
      }
    }
