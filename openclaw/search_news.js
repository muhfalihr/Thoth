...
const { outPath, cropPath } = require('./paths');

const args = process.argv.slice(2);
const getFlag = (n, d) => { const i = args.indexOf(n); return i >= 0 ? args[i + 1] : d; };
const QUERY = args.find(a => !a.startsWith('--') && !['--max', '--mode', '--append'].includes(args[args.indexOf(a) - 1]));
...
if (!QUERY) { console.log('Usage: node search_news.js "<topik>" [--max N] [--mode chart|articles|auto]'); process.exit(1); }
...
const CUR = {
  'dolar amerika': 'USD', 'dolar singapura': 'SGD', 'dolar australia': 'AUD', 'mata uang rupiah': 'IDR',
  rupiah: 'IDR', idr: 'IDR', dolar: 'USD', dollar: 'USD', usd: 'USD', euro: 'EUR', eur: 'EUR',
  yen: 'JPY', jpy: 'JPY', ringgit: 'MYR', myr: 'MYR', won: 'KRW', krw: 'KRW', yuan: 'CNY', cny: 'CNY',
  renminbi: 'CNY', poundsterling: 'GBP', pound: 'GBP', gbp: 'GBP', sterling: 'GBP', baht: 'THB',
  peso: 'PHP', riyal: 'SAR', sar: 'SAR', dirham: 'AED', aed: 'AED', franc: 'CHF', chf: 'CHF', aud: 'AUD', sgd: 'SGD',
};
...
function detectChartQuery(q) {
  ...
  const codes = []; hits.sort((a, b) => a.idx - b.idx).forEach(h => { if (!codes.includes(h.code)) codes.push(h.code); });
  if (!codes.length) return null;
  let base, quote;
  if (codes.length >= 2) { base = codes.includes('USD') ? 'USD' : codes[0]; quote = codes.find(c => c !== base); }
  else { if (codes[0] === 'USD') return null; base = 'USD'; quote = codes[0]; }
...
async function captureTagged(client, tag, outFile, pad = 10) {
  ...
  const measure = async () => {
    await client.evaluate(`(() => { const el = document.querySelector('[data-news-crop="${tag}"]'); if (el) el.scrollIntoView({block:"center"}); })()`);
    await sleep(450);
    const rj = await client.evaluate(`(() => { const el = document.querySelector('[data-news-crop="${tag}"]'); if (!el) return ''; const r = el.getBoundingClientRect(); return JSON.stringify({x:r.x+window.scrollX,y:r.y+window.scrollY,w:r.width,h:r.height}); })()`);
...
async function doChart(client, chartQuery) {
  ...
  await client.navigate('https://www.google.com/search?hl=id&q=' + encodeURIComponent(chartQuery), 6000);
  await sleep(2500);
  let tagged = 0;
  for (let t = 0; t < 6; t++) { tagged = await client.evaluate(CHART_FIND); if (tagged) break; await sleep(1000); }
  if (!tagged) { console.log('  ⚠️ widget kurs tak ketemu (mungkin Google tak menampilkan converter utk query ini).'); return null; }
  const value = await client.evaluate(`(() => { const el = document.querySelector('[data-news-crop="chart"]'); if (!el) return ''; const line = (el.innerText||'').split('\\n').map(s=>s.trim()).find(s=>/=|Rp|\\d[\\d.,]+/.test(s)); return (line||'').slice(0,80); })()`);
  const img = await captureTagged(client, 'chart', cropPath(`news_chart_${slug}.png`));
  if (!img) { console.log('  ⚠️ crop chart gagal/blank.'); return null; }
  console.log(`  ✅ ${path.basename(img)}  | nilai: ${value}`);
  return { chart_query: chartQuery, value, image_path: img, source_url: 'https://www.google.com/search?q=' + encodeURIComponent(chartQuery) };
}
...
async function doArticles(client, max) {
  ...
  for (const it of items) {
    const img = await captureTagged(client, 'a' + it.idx, cropPath(`news_${slug}_${it.idx + 1}.png`));
    out.push({ title: it.title, url: it.url, source: it.source, image_path: img || '' });
    console.log(`  ${it.idx + 1}. ${img ? '✅' : '⚠️ (no crop)'} ${it.title.slice(0, 60)} — ${it.source}`);
  }
  return out;
}
...
run(async () => {
  ...
  const result = { query: QUERY, fetched_at: new Date().toISOString(), mode: '', chart: null, articles: [] };
  ...
  if (APPEND) {
    if (!fs.existsSync(APPEND)) { console.log(`⚠️  --append: ${APPEND} tak ada, lewati.`); return; }
    let set; try { set = JSON.parse(fs.readFileSync(APPEND, 'utf8')); } catch (e) { console.log('⚠️  --append: JSON tak valid.'); return; }
    set.footage = set.footage || [];
    let added = 0;
    const push = (url, image_path) => { if (image_path) { set.footage.push({ url, platform: 'news', query: QUERY, is_video: false, image_path, relevance: 'match' }); added++; } };
