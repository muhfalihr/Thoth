...
const { verifyTikTok } = require('./verify');
const { outPath } = require('./paths');

async function main() {
  const query = process.argv[2];
  const keywords = process.argv.slice(3);
  if (!query) { console.log('Usage: node search_tiktok_v2.js "<query>" [keyword ...]'); process.exit(1); }
  ...
  await client.scroll(1200); await sleep(1500);
  await client.scroll(2400); await sleep(1500);
  ...
  if (keywords.length) {
    ...
    for (const u of valid.slice(0, 20)) {
      ...
      console.log(`   ${mark} [${rel}] ${u}\n        "${(r.caption || '(no caption)').slice(0, 70)}"`);
      if (rel !== 'off-topic') out.push({ url: u, relevance: rel, caption: r.caption, author: r.author });
    }
  ...
  fs.writeFileSync(outFile, JSON.stringify({ query, keywords, fetched_at: new Date().toISOString(), urls: out }, null, 2));
  console.log(`\n💾 ${outFile}`);
  if (!out.length) console.log('⚠️  Nol URL. Jangan rakit URL manual — pastikan tab TikTok login ter-attach & query benar.');
}
